...
# Direct UDP Socket Initializer
sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
current_cmd = "S"

def send_udp_cmd(cmd):
    """Sends immediate zero-latency UDP packet to the ESP32."""
    global current_cmd
    if cmd != current_cmd:
        current_cmd = cmd
        try:
...
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(
        f"Model file '{MODEL_PATH}' not found! Download it via: "
...
def is_fist_geometric(landmarks):
    tips = [8, 12, 16, 20]
    pips = [6, 10, 14, 18]
    return sum(1 for tip, pip in zip(tips, pips) if landmarks[tip].y > landmarks[pip].y) >= 3
...
print("Starting ESP32 Gesture Steering... Press 'q' or 'ESC' to exit.")
...
while cap.isOpened():
    ...
    # Gestures Logic Routing
    if num_hands == 1:
        # SINGLE HAND DETECTED -> STOP
        hand_lm = results.hand_landmarks[0]
        g_name = results.gestures[0][0].category_name if results.gestures else "None"
        if (g_name in ["Open_Palm", "palm"]) or is_palm_geometric(hand_lm):
            command_to_send = "S"
            status_text = "SINGLE PALM (STOP)"
        
        for pt in hand_lm:
            cv2.circle(frame, (int(pt.x * w), int(pt.y * h)), 4, (0, 0, 255), -1, cv2.LINE_AA)

    elif num_hands == 2:
        hand1_lm, hand2_lm = results.hand_landmarks[0], results.hand_landmarks[1]
        ...
        # 1. BOTH HANDS FISTS -> FORWARD / STEERING
    ...
    # Dispatch Command to ESP32 over UDP
    send_udp_cmd(command_to_send)

    # Display HUD
    color = (0, 0, 255) if command_to_send == "S" else (0, 255, 0)
    cv2.putText(frame, f"STATUS: {status_text}", (40, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.1, color, 3, cv2.LINE_AA)
    cv2.putText(frame, f"CMD: {command_to_send}", (40, 115), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 3, cv2.LINE_AA)
